...
Node 22 · TypeScript (strict) · Vite 8 · React 19.2.0 (pinned exactly) · three.js 0.186 ·
...
- All user-visible strings live in `src/data/` (`dialogue.ts` for UI and speech, `quiz.ts`,
  `codex.ts`) behind a key. No user-visible string literals anywhere else.
...
- `public/assets/` — generated by `tools/` and **committed** (raw/ exists only on one
  machine); regenerate with `npm run assets:build`, never edit by hand
